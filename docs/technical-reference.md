...
**Category value delimiter contract:** Category facet parsing supports both comma-delimited and semicolon-delimited multi-category `raw.category` values (for example: `"Fraud and corruption, Finance and travel"` and `"Fraud and corruption; Finance and travel"`). For comma-delimited values, a capitalization rule is applied: split only when the next non-space character is uppercase. This keeps labels like `"Conduct, integrity and risk"` as a single category value.
...
| `.doc-search-filter-group`                         | A single filter section container (Type, Category, Content owner, etc.)                                                                                                                                                                                                                            |
| `.doc-search-filter-group__title`                  | Non-collapsible facet group heading `<h3>` or `<label>` (Type, Category, Content owner); `font-size: 16px !important`; `margin: 0 !important`                                                                                                                                                      |
