...
**Category value delimiter contract:** Category facet parsing treats `raw.category` as a semicolon-delimited list for multi-category documents (for example: `"Fraud and corruption; Governance and accountability"`). Commas are treated as literal punctuation inside a single category label, so values like `"Conduct, integrity and risk"` remain intact and are not split.
