...
9. On `internal.nt.gov.au` pages, apply a base-prefix visibility rule in the rendered DOM: hide `<a>` links whose base prefix differs from the current page base prefix (`scheme + host + first path segment`, e.g. `https://internal.nt.gov.au/dcdd`)
...
- `filterPageLinksByPrefix($container)` — host-gated DOM post-filter that runs only when `window.location.hostname === "internal.nt.gov.au"`. After links are rendered, it hides anchors whose `href` base prefix does not match the current page base prefix.
