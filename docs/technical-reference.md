...
- `input[name="doc-search-drawer-sort"]` — drawer copy of the Sort by radio inputs; separate name keeps drawer changes staged until Apply
...
- `#doc-search-drawer-clear` — "Clear all" button in the drawer body — resets sort + all checkboxes
...
| `.doc-search-results-header`                       | Bar above results — summary text + controls. Uses `flex-direction: column` to stack summary text and controls row.                                                                                                                                                                                 |
| `.doc-search-results-summary`                      | "Showing X–Y of N results" `<p>` placed in its own row above controls.                                                                                                                                                                                                                             |
| `.doc-search-results-controls`                     | Flex row — contains the right-aligned view toggle button.                                                                                                                                                                                                                                          |
| `.doc-search-view-toggle`                          | Card/table toggle pill `<button>`                                                                                                                                                                                                                                                                  |
...
- **Sort is client-side; filters are preserved on sort change.** Changing the desktop sidebar sort radios (`input[name="doc-search-sort"]`) calls `applySort()` then `applyFilters()` — no API call, no filter reset. `originalResults` always holds the unmodified API response so "Relevance" can restore it cheaply. The mobile drawer has its own radio group (`input[name="doc-search-drawer-sort"]`) that mirrors the active sort when the drawer opens; selecting a drawer radio is staged until "Apply filters" syncs it back to desktop state and applies sorting/filtering.

- **Mobile filter drawer.** On screens ≤ 900 px the filter sidebar is hidden and replaced by a "Filter and sort" pill button (`#doc-search-mobile-filter-btn`). Tapping it opens a slide-in drawer (`#doc-search-drawer`, `position: fixed`, slides from the right) with the full sort + facet UI duplicated. The drawer has its own sort radio group (`name="doc-search-drawer-sort"`), its own Type/Category checkbox lists, and its own Content owner dropdown select (`name="doc-search-drawer-owner"`). On "Apply filters" (`#doc-search-drawer-apply`), `coveo-search.js` reads the drawer sort selection, syncs it to the sidebar sort radios, reads the drawer checkboxes and Content owner dropdown selection, syncs them to the sidebar checkboxes/Content owner select, then calls `applySort()` + `applyFilters()`. On "Clear all" (`#doc-search-drawer-clear`), sort resets to `relevancy`, Content owner select resets to `""`, and all checkboxes are unchecked in the drawer without applying until the user taps Apply. The desktop sidebar also has its own "Clear all" button (`#doc-search-sidebar-clear`) which immediately resets all filters and re-applies them. The overlay and close button both fire the same close routine.

  **Live item count in the drawer footer:** The drawer footer contains a `<p id="doc-search-drawer-item-count">` element above the "Apply filters" button that shows how many results the current drawer filter selections would yield (e.g. "42 items found" or "1 item found"). `updateDrawerItemCount()` reads the drawer checkboxes and owner select, runs the same AND/OR filter logic as `applyFilters()` against `allResults` (without mutating `filteredResults` or triggering a render), and sets the count text. It is called on drawer open (after `buildDrawerFilters()`), on every drawer checkbox change, on drawer owner select change, and on drawer "Clear all" — giving the user immediate feedback before committing with "Apply filters".
