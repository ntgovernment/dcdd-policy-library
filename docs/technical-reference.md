...
- The production source markup keeps that container empty.
- The local dev preview adds a DCDD notice block in `search-section-preview.html`, so the preview shows the message without needing injected Matrix content.
- `search-section-preview.html` is a local preview artifact only and is not deployed to Matrix.
...
2. **`auto-rebuild-on-src-change`** (`configureServer` hook, dev server only): watches `src/js/**/*.js`, `src/css/**/*.css`, and `src/*.html`. Behaviour varies by file type:
   - **JS or CSS change:** runs Vite's programmatic `build()` to regenerate `dist/search-page.js` and `dist/search-page.css` (or collection equivalents), then sends a full browser reload. A `building` guard prevents concurrent builds. Logged as `[auto-rebuild] <file> changed — rebuilding…`.
   - **HTML change (`src/*.html`):** recopies `src/search-section.html` and `src/search-results.html` to `dist/`, calls `syncPreviewTemplate()`, regenerates the pages using `scripts/generate-collection-pages.js`, then sends a full browser reload — no JS bundle rebuild needed. Logged as `[auto-rebuild] <file> changed — HTML recopied and pages generated, reloading browser`.

3. **`syncPreviewTemplate()`** (called by both of the above): reads the entirety of `src/search-results.html`, re-indents it to the 8-space indent used in `search-section-preview.html`, and replaces the matching search UI block from `<div class="doc-search-outer">` through the preview page's search bundle script tag. This keeps the dev preview's search results layout, filters, card template, and adjacent search UI markup (such as the view preference modal) in sync with the canonical source automatically — you never need to manually edit `search-section-preview.html` for structural changes to the search interface.
...
2. **Appends a text fragment** — `formatFileMeta(raw)` derives the file-type/size metadata (e.g. `DOCX (612.4 KB)`) and encodes it as a [text fragment](https://developer.mozilla.org/en-US/docs/Web/URI/Fragment/Text_fragments) appended to the URL:
   ```
...
// Persists view + items preferences to user metadata fields #969752 and #980780.
```

### Optional standalone patch: search preference metadata persistence

`src/js/view-preference-metadata-patch.js` is a standalone IIFE patch that syncs search preferences with Squiz user metadata.

- View metadata field ID: `969752` (`user.view-preference`)
- View persisted values: `grid` or `table` (default when no preference exists)
- Items metadata field ID: `980780` (`user.items-preference`)
- Items persisted values: `10`, `20`, or `all` (default when no preference exists: `10`)
- Compatibility keys: `localStorage.docSearchView` (`card`/`table`) and `localStorage.docSearchItemsPerPage` (`10`/`20`/`all`)
...
3. The **Showing** dropdown preference is read before initial render and written immediately on change (`10`, `20`, `all`).
4. On mobile (`max-width: 900px`), UI stays in card mode while the saved desktop preference is preserved.
...
│   │   └── view-preference-metadata-patch.js # Bundled startup provider: metadata-first view resolution + persistence #969752
│   ├── css/
...
| --------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `masterResults`             | Array   | Complete document corpus (all results for an empty query); populated once on the first `runSearch()` call and never cleared; provides the stable value list for all facets so Type/Topic options do not disappear when a query narrows the result set |
...
| `activeTopicFilters`        | Set     | Checked values under the Topic facet                                                                                                                                                                                                                  |
...
| `search-result-title`           | `raw.resourcefriendlytitle \|\| result.title` with `formatFileMeta()` metadata rendered alongside the title — e.g. `"My Document PDF (354.2 KB)"`, `"My Document DOCX"`, or `"My Document (58.5 KB)"` when type/size fields are present     |
...
**Topic value delimiter contract:** Topic facet parsing supports both comma-delimited and semicolon-delimited multi-topic `raw.topic` values (for example: `"Fraud and corruption, Finance and travel"` and `"Fraud and corruption; Finance and travel"`). For comma-delimited values, a capitalization rule is applied: split only when the next non-space character is uppercase. This keeps labels like `"Conduct, integrity and risk"` as a single topic value.
...
**Sort change behaviour:** Changing the sort radio buttons (sidebar: `input[name="doc-search-sort"]`; mobile drawer: `input[name="doc-search-drawer-sort"]`) calls `applySort()` then `applyFilters()` — **no Coveo API re-fetch, no network request**. Active Type and Category filter checkboxes are preserved. Four options: **Relevance** (`relevancy`) restores the original API response order (`originalResults`); **Last updated** (`date descending`) resolves date values from `approveddate` with fallback to `resourceupdated` and sorts `allResults` chronologically descending; **A – Z** (`alpha ascending`) and **Z – A** (`alpha descending`) sort by `raw.resourcefriendlytitle` (falling back to `result.title`) using `String.localeCompare()`.

**Items-per-page behaviour:** The pagination row includes a right-side **Showing** dropdown (`#doc-search-items-per-page`) with shared options `10`, `20`, and `all` for both card and table views. Changing this value re-renders from page 1 without triggering a Coveo API request. When the filtered result count fits on a single page for the current selection, the pagination control remains visible as a muted disabled shell (`Prev`, `1`, `Next`) for layout consistency; choosing `all` guarantees that single-page state because every filtered result is rendered at once. The selected value is persisted immediately to metadata field `#980780` (`user.items-preference`) and restored before first render.
...
- `#doc-search-filter-toast` — desktop-only transient toast/live region used for immediate-apply filter feedback; announces the applied result count for sidebar Type/Topic/Content owner changes and desktop "Clear all". Positioned to the left edge of the desktop filter sidebar and, when the footer enters the viewport, lifted above `.ntgc-footer` so it does not overlap the footer.
- `#doc-search-results-summary` — "Showing X–Y of N results" text; when a query exists it appends `for "query"` with only the query term bolded; aligned left in the same header row as the description toggle
- `#doc-search-mobile-filter-btn` — "Filters" pill button (hidden on desktop, visible ≤ 900 px); slides in the filter drawer
- `#doc-search-view-toggle` — **Show description** pill button (`aria-pressed="true"` = card view with descriptions active, `aria-pressed="false"` = table view)
...
- `#doc-search-sidebar` — filter sidebar containing the desktop Sort by radio group above the Filters heading, followed by Type, Topic, and Content owner filter groups; hidden on mobile
...
- `#doc-search-topic-filters` — `<ul>` of checkbox filter items for `topic`
...
- `input[name="doc-search-drawer-sort"]` — drawer copy of the Sort by radio inputs; separate name keeps drawer changes staged until Show N results
- `#doc-search-drawer-type-filters` — drawer copy of the Type facet checkbox list
- `#doc-search-drawer-topic-filters` — drawer copy of the Topic facet checkbox list
- `#doc-search-drawer-clear` — "Clear all" secondary link in the drawer footer — resets staged sort, owner, and checkboxes
- `#doc-search-drawer-apply` — dynamic "Show N results" button in the drawer footer — syncs drawer state back to sidebar
...
| `#doc-search-filter-toast`             | Desktop-only transient toast/live region (`role="status"`, `aria-live="polite"`) used for immediate filter feedback; hidden on mobile and not reused for persistent errors/no-results |
| `#doc-search-results-summary`          | "Showing X–Y of N results" line; with a query it becomes `Showing X–Y of N results for "query"`, with only the query text rendered in `<strong>`                     |
...
| `#doc-search-topic-filters`            | Topic facet checkbox list (sidebar)                                                                                                                                  |
...
| `.doc-search-results-col`                          | Results column; `[data-view="pending"]` hides both result presentations during startup, `[data-view="table"]` activates table mode, `[data-view="card"]` activates card mode                                                                                                                            |
...
| `.doc-search-results-summary`                      | "Showing X–Y of N results" `<p>` in the shared results header row; with a query it appends `for "<strong>query</strong>"`; margin is reset so inherited paragraph styles do not disrupt alignment.                                                                                                      |
...
| `.doc-search-filter-toast`                         | Desktop-only transient toast/live-region surface. `coveo-search.js` positions it from the desktop sidebar's left edge rather than the viewport edge, refreshes that position on scroll/resize while visible, and swaps from `bottom` anchoring to a computed `top` position when `.ntgc-footer` enters the viewport so the toast sits above the footer instead of overlapping it. |
...
| `.doc-search-sidebar__header`                      | Wrapper for sidebar heading and clear button                                                                                                                                                                                                                                                            |
| `.doc-search-sidebar__heading`                     | Sidebar header `<h2>` ("Filters") with bold typography.                                                                                                                                                                                                                                                 |
| `.doc-search-sidebar__clear`                       | "Clear all" `<button>` in the sidebar header                                                                                                                                                                                                                                                            |
| `.doc-search-filter-group`                         | A single filter section container (Type, Topic, Content owner, etc.)                                                                                                                                                                                                                                    |
| `.doc-search-filter-group__title`                  | Non-collapsible facet group heading `<h3>` or `<label>` (Type, Topic, Content owner); `font-size: 16px !important`; `margin: 0 !important`                                                                                                                                                              |
| `.doc-search-filter-group--sort-radios`            | Fieldset modifier for the expanded Sort by radio group used in the desktop sidebar and mobile drawer. The group title (`> .doc-search-filter-group__title`) is intentionally overridden to `font-size: 20px !important` (matching `.doc-search-sidebar__heading`) with `margin-bottom: 1em !important`. |
| `.doc-search-sort-radio-list`                      | Vertical wrapper for Sort by radio labels.                                                                                                                                                                                                                                                              |
| `.doc-search-sort-radio`                           | Sort by radio label wrapper.                                                                                                                                                                                                                                                                            |
| `.doc-search-sort-radio__input`                    | Custom-styled radio input for sorting.                                                                                                                                                                                                                                                                  |
| `.doc-search-sort-radio__label`                    | Visible sort option label.                                                                                                                                                                                                                                                                              |
| `.doc-search-sort-select`                          | Wrapper `<div>` for dropdown selects such as Content owner; retained for select controls that still use custom chevrons.                                                                                                                                                                                |
| `.doc-search-sort-select__input`                   | `<select>` input styling retained for select controls.                                                                                                                                                                                                                                                  |
...
| `.doc-search-mobile-filter-btn__label`             | "Filter and sort" text `<span>` inside the mobile button                                                                                                                                                                                                                                                |
...
| `.doc-search-drawer__body`                         | Scrollable region containing the sort group, Type facet, Topic facet, and Content owner select                                                                                                                                                                                                          |
...
| `.doc-search-filter-group--owner`                  | Content owner filter group container (`gap: 8px`)                                                                                                                                                                                                                                                       |
| `.doc-search-owner-select`                         | Content owner dropdown wrapping div                                                                                                                                                                                                                                                                     |
...
- **Font Awesome is still required for the search form icon.** The search submit icon uses `<span class="fal fa-search">` in `src/search-section.html`, so Font Awesome Pro 5 CSS must be present on the page (typically via Matrix paint layout). Other UI icons in this widget remain inline SVG or CSS-based (for example: pagination chevrons, filter/drawer icons, spinner ring).
...
- **Sort is client-side; filters are preserved on sort change.** Changing the desktop sidebar sort radios (`input[name="doc-search-sort"]`) calls `applySort()` then `applyFilters()` — no API call, no filter reset. `originalResults` always holds the unmodified API response so "Relevance" can restore it cheaply. The mobile drawer has its own radio group (`input[name="doc-search-drawer-sort"]`) that mirrors the active sort when the drawer opens; selecting a drawer radio is staged until "Show N results" syncs it back to desktop state and applies sorting/filtering.

- **Mobile filter drawer.** On screens ≤ 900 px the filter sidebar is hidden and replaced by a "Filter and sort" pill button (`#doc-search-mobile-filter-btn`). Tapping it opens a slide-in drawer (`#doc-search-drawer`, `position: fixed`, slides from the right) with the full sort + facet UI duplicated. The drawer has its own sort radio group (`name="doc-search-drawer-sort"`), its own Type/Category checkbox lists, and its own Content owner dropdown select (`name="doc-search-drawer-owner"`). On "Show N results" (`#doc-search-drawer-apply`), `coveo-search.js` reads the drawer sort selection, syncs it to the sidebar sort radios, reads the drawer checkboxes and Content owner dropdown selection, syncs them to the sidebar checkboxes/Content owner select, then calls `applySort()` + `applyFilters()`. The fixed footer keeps that primary action and the secondary "Clear all" link visible while the drawer body scrolls. On "Clear all" (`#doc-search-drawer-clear`), sort resets to `relevancy`, Content owner resets to `""`, and all drawer checkboxes are unchecked without applying until the user selects "Show N results". The desktop sidebar also has its own "Clear all" button (`#doc-search-sidebar-clear`) which immediately resets all filters and re-applies them. The overlay and close button both fire the same close routine.

  **Live result count in the primary action:** `updateDrawerItemCount()` sets the primary button label to "Show 1 result" or "Show N results". It reads the drawer checkboxes and owner select, runs the same AND/OR filter logic as `applyFilters()` against `allResults` without mutating `filteredResults` or triggering a render, and updates on drawer open (after `buildDrawerFilters()`), every drawer checkbox change, drawer owner change, and drawer "Clear all". This gives immediate feedback before the staged selections are committed. The static HTML fallback label is "Show results" and is replaced as soon as `updateDrawerItemCount()` runs.

  **Desktop filter toast:** Immediate-apply desktop filter changes surface the applied result count in `#doc-search-filter-toast`, a transient polite live region. The toast is triggered only for desktop sidebar checkbox changes, desktop Content owner changes, and desktop "Clear all". It is intentionally not shown on mobile, including staged drawer changes and drawer apply, because the drawer already communicates staged counts via `#doc-search-drawer-apply`.
...
- **Desktop filter toast positioning is runtime-computed.** `#doc-search-filter-toast` is not permanently anchored by static CSS alone. While visible, `coveo-search.js` positions it from the left edge of `#doc-search-sidebar`, clamps it within the viewport, and recomputes it on scroll/resize. When `.ntgc-footer` enters the viewport, the script switches the toast from bottom anchoring to a computed top position so the toast sits above the footer instead of overlapping it. If the sidebar width, outer layout width, or footer selector changes, review the toast positioning helper in `src/js/coveo-search.js`.
